...
void ADS_writeRegister(uint16_t command, ADS_8688 *ads) {
	ads8688.datTx[0] = command;
	ads8688.datTx[1] = 0x0000;
	ads8688.datRx[0] = 0x0000;
	ads8688.datRx[1] = 0x0000;

	HAL_GPIO_WritePin(ads->cs_port, ads->cs_pin, GPIO_PIN_RESET);
	HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(ads->hspi, (uint8_t*) ads8688.datTx, (uint8_t*) ads8688.datRx, 2, 100);
	HAL_GPIO_WritePin(ads->cs_port, ads->cs_pin, GPIO_PIN_SET);

	ADS_appendData("SPI command: ", 13, ads8688.datTx[0]);
	ADS_appendData("SPI Status: ", 12, status);
	ADS_appendData("Data Written: ", 14, ads8688.datRx[1]);
}
...
void ADS_measure(ADS_8688 *ads) {
	ads8688.datTx[0] = 0x0000;
	ads8688.datTx[1] = 0x0000;
	ads8688.datRx[0] = 0x0000;
	ads8688.datRx[1] = 0x0000;

	HAL_GPIO_WritePin(ads->cs_port, ads->cs_pin, GPIO_PIN_RESET);
	HAL_SPI_TransmitReceive(ads->hspi, (uint8_t*) ads8688.datTx, (uint8_t*) ads8688.datRx, 2, 100);
	HAL_GPIO_WritePin(ads->cs_port, ads->cs_pin, GPIO_PIN_SET);

	ads->dataBuffer = ads8688.datRx[1];
}
